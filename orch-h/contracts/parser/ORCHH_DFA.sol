...
/// @title ORCH-H DFA Parser
/// @notice Validates ORCH-H bytecode before execution
library ORCHH_DFA {
    ...
    function validate(bytes calldata program) internal pure {
        ...
        for (uint256 i = 0; i < program.length; i++) {
            ...
            if (op == OP_RAID) {
                if (state != State.START && state != State.RAIDING)
                ...
                if (state != State.RAIDING && state != State.EXECUTING)
                    revert InvalidTransition();
                state = State.EXECUTING;
            }

            else if (op == OP_ASSERT) {
                if (state != State.EXECUTING && state != State.ASSERTING)
                    revert InvalidTransition();
                state = State.ASSERTING;
            }

            else if (op == OP_WITHDRAW) {
                if (state != State.ASSERTING && state != State.REPAYING)
                    revert InvalidTransition();
